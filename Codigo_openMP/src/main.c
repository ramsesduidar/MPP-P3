...
extern double aplicar_ga(const double *, int, int, int, double, int *, int);
...
//	Check Number of Input Args
	if(argc < 5) {
		...
		fprintf(stderr,"  ./programa n nGen tamPob mRate nhilos\n");
		return(EXIT_FAILURE);
	}
...
//	Allocate memory for output data
	...
	#endif
//	Call Metaheuristic
	double value = aplicar_ga(d, n, n_gen, tam_pob, m_rate, sol, nh);
