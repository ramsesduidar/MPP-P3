...
double aplicar_ga(int name, int p, const double *d, int n, int n_gen, int tam_pob, double m_rate, int ngm, int nem, int *sol, int nh)
{
	...
	for(g = 0; g < n_gen; g++)
	{
		...
		for(i = 0; i < (new_tam/2) - 1; i += 2) {
			cruzar(poblacion[i], poblacion[i+1], poblacion[new_tam/2 + i], poblacion[new_tam/2 + i + 1], n, nh);
		}
		...
		{
			#pragma omp single nowait
			mergeSort(poblacion, 0, new_tam, n);
		}
