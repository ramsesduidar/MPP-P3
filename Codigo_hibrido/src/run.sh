...
while read line ; do 
		...
		nh=$(echo $line | tr -s ' ' | cut -f7 -d ' ')
	
		echo -e >> $out_file
		echo -n "Executing with: " >> $out_file
		echo -e "N = "$n" N_GEN = "$gen" TAM_POB = "$tam" M_RATE = "$m_rate" NGM = "$ngm" NEM = "$nem" N_HILOS = "$nh >> $out_file
		make test_sec N=$n N_GEN=$gen T_POB=$tam M_RATE=$m_rate NGM=$ngm NEM=$nem N_HILOS=$nh
done < $in_file
