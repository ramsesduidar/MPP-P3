...
double aplicar_ga(const double *d, int n, int n_gen, int tam_pob, double m_rate, int *sol, int NHI, int NHF)
{
	...
	omp_set_nested(1);
	#pragma omp parallel for num_threads(NHI) //schedule(static, 1)
	for(i = 0; i < tam_pob; i++) {
	...
	for(g = 0; g < n_gen; g++)
	{
		...
		#pragma omp parallel for num_threads(4) schedule(dynamic, tam_pob/24) 
...
void fitness_reduction(const double *d, Individuo *individuo, int n, int NHF)
{
	double fitness = 0.0;
	#pragma omp parallel num_threads(NHF)
	{
