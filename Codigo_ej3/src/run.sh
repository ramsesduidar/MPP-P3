...
while read line ; do 
		...
		nhi=$(echo $line | tr -s ' ' | cut -f5 -d ' ')
		nhf=$(echo $line | tr -s ' ' | cut -f6 -d ' ')
	
		#echo -e >> $out_file
		#echo -n "Executing with: " >> $out_file
		#echo -e "N = "$n" N_GEN = "$gen" TAM_POB = "$tam" M_RATE = "$m_rate" NHI = "$nhi" NHF = "$nhf >> $out_file
		make test_sec N=$n N_GEN=$gen T_POB=$tam M_RATE=$m_rate NHI=$nhi NHF=$nhf
done < $in_file
